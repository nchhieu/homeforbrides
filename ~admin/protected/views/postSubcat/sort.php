    ...
    <div id="reorder-gallery">
      <ul id="sortable">
        <?php
				...
				foreach($data as $row){
    			?>
        <li id="ID_<?php echo $row['subcat_id'];?>"><img src="<?php echo Yii::app()->params['domain'].$row['subcat_thumb'];?>" title="<?php echo $row['subcat_name'];?>" alt="<?php echo $row['subcat_name'];?>" /></li>
        <?php
				} // for
     			?>
      </ul>
...
$(function() {
  $("#sortable").sortable({
				...
				update: function(){
						...
						jQuery.post("index.php?r=PostSubcat/UpdateSort", order, 
