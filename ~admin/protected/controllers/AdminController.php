...
class AdminController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Admin']))
		{
			...
			$rand = rand(0, 999);
			if ($admin_avatar) {$model->admin_avatar = '/photos/' . date('Ym') . '/' .   $model->admin_email . '_' . $rand . '.' . $admin_avatar->extensionName;}
			if($model->validate()){
				if($model->save()){
					if ($admin_avatar) {$admin_avatar->saveAs('../photos/' .  date('Ym') . '/' . $model->admin_email . '_' . $rand . '.' . $admin_avatar->extensionName);}
